// Tamil Nadu local bodies master dataset — aligned with EDM repository
// Contains Corporations, Municipalities, Town Panchayats, and all 38 districts.

const ALL_DISTRICTS = ['Ariyalur','Chengalpattu','Chennai','Coimbatore','Cuddalore','Dharmapuri','Dindigul','Erode','Kallakurichi','Kanchipuram','Kanyakumari','Karur','Krishnagiri','Madurai','Mayiladuthurai','Nagapattinam','Namakkal','Nilgiris','Perambalur','Pudukkottai','Ramanathapuram','Ranipet','Salem','Sivaganga','Tenkasi','Thanjavur','Theni','Thoothukudi','Tiruchirappalli','Tirunelveli','Tirupathur','Tiruppur','Tiruvallur','Tiruvannamalai','Tiruvarur','Vellore','Viluppuram','Virudhunagar'];
...
function getLocalBodiesForDistrict(districtName, type) {
  const cleanDist = (districtName || 'Tiruvallur').trim().toLowerCase();
  const matchedDistrict = ALL_DISTRICTS.find(d => d.toLowerCase() === cleanDist) || 'Tiruvallur';
  const isUrban = (type || 'rural').toLowerCase() === 'urban';

  let list = [];
  if (isUrban) {
    const corps = LOCAL_BODIES.corporations
      .filter(b => b.district.toLowerCase() === matchedDistrict.toLowerCase())
      .map(b => ({ ...b, type: 'Corporation' }));
    const muns = LOCAL_BODIES.municipalities
      .filter(b => b.district.toLowerCase() === matchedDistrict.toLowerCase())
      .map(b => ({ ...b, type: 'Municipality' }));
    const tps = LOCAL_BODIES.town_panchayats
      .filter(b => b.district.toLowerCase() === matchedDistrict.toLowerCase())
      .map(b => ({ ...b, type: 'Town Panchayat' }));

    list = [...corps, ...muns, ...tps];
    if (list.length === 0) {
      list = [
        { district: matchedDistrict, name: `${matchedDistrict} Municipality`, wards: 30, type: 'Municipality' },
        { district: matchedDistrict, name: `${matchedDistrict} Town Panchayat`, wards: 18, type: 'Town Panchayat' }
      ];
    }
  } else {
    list = [
      { district: matchedDistrict, name: `${matchedDistrict} Central Panchayat Union`, wards: 20, type: 'Panchayat Union' },
      { district: matchedDistrict, name: `${matchedDistrict} North Gram Panchayat`, wards: 15, type: 'Gram Panchayat' },
      { district: matchedDistrict, name: `${matchedDistrict} South Gram Panchayat`, wards: 15, type: 'Gram Panchayat' },
      { district: matchedDistrict, name: `${matchedDistrict} District Panchayat`, wards: 30, type: 'District Panchayat' }
    ];
  }

  return list.map((item, idx) => {
    const typeLabel = item.type || (isUrban ? 'Municipality' : 'Panchayat Union');
    const hasTypeInName = item.name.toLowerCase().includes(typeLabel.toLowerCase());
    return {
      id: `lb_${matchedDistrict.toLowerCase()}_${idx + 1}`,
      name: hasTypeInName ? item.name : `${item.name} ${typeLabel}`,
      type: typeLabel,
      wards: item.wards || 20
    };
  });
